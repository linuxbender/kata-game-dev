...
export const combineHudRenderers = (renderers: HudRenderer[]): HudRenderer => {
  return (ctx, world, camX, camY, viewW, viewH, dpr) => {
    for (const r of renderers) {
      try { r(ctx, world, camX, camY, viewW, viewH, dpr) } catch (e) { /* swallow to avoid breaking frame */ }
    }
  }
}
...
export const HudOverlay: React.FC<{ world: TypedWorld | null; player: number | null }> = ({ world, player }) => {
  ...
  useEffect(() => {
    ...
    try { console.debug('[HUD] HudOverlay mounted for player', player) } catch {}
    // Initialize
    ...
    return () => {
      unsub()
      try { console.debug('[HUD] HudOverlay unmounted for player', player) } catch {}
  ...
  return (
    <div style={{ minWidth: 140, padding: 6, background: 'rgba(0,0,0,0.4)', color: '#fff', borderRadius: 6 }}>
      ...
      <div style={{ width: 120, height: 12, background: 'rgba(255,255,255,0.12)', borderRadius: 6 }}>
        <div style={{ height: '100%', width: `${(displayValue.current / displayValue.max) * 100}%`, background: '#e74c3c', borderRadius: 6, transition: 'width 120ms linear' }} />
      </div>
